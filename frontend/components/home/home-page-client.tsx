...
const copy = {
  heroTitle: "\u5168\u6808\u5f00\u53d1\u8005",
  heroTitleEn: "Full-Stack Developer",
  heroSubtitle: "\u6784\u5efa\u5185\u5bb9\u4e0e\u4f53\u9a8c\u7684\u6570\u5b57\u7a7a\u95f4",
  heroSubtitleEn: "Building content-driven experiences",
  heroIntro:
    "\u8fde\u63a5\u535a\u5ba2\u3001\u5b9e\u9a8c\u5ba4\u4e0e\u6253\u5361\u5e73\u53f0\uff0c\u6253\u9020\u6301\u7eed\u8f93\u51fa\u7684\u5f00\u53d1\u8005\u4f5c\u54c1\u96c6\u3002",
  ctaEnter: "\u8fdb\u5165\u7ad9\u70b9",
  ctaEnterEn: "Enter Site",
  ctaBlog: "\u9605\u8bfb\u535a\u5ba2",
  ctaBlogEn: "Blog",
      ...
      <section className="grid gap-10 md:grid-cols-[1.2fr_0.8fr] md:items-center">
        <div className="space-y-6">
          <div className="space-y-4">
            <h1 className={heroMotion("delay-0")}>
              {copy.heroTitle}
              <span
                className="mt-2 block text-lg font-medium text-slate-400 md:text-xl"
                lang="en"
              >
                / {copy.heroTitleEn}
              </span>
            </h1>
            <p className={`${heroMotion("delay-100")} text-base text-slate-300`}>
              {copy.heroSubtitle}
              <span className="mt-1 block text-sm text-slate-400" lang="en">
                {copy.heroSubtitleEn}
              </span>
            </p>
            <p className={`${heroMotion("delay-200")} text-sm text-slate-400`}>
              {copy.heroIntro}
            </p>
          </div>
        ...
        <div
          className={`${heroMotion(
            "delay-200"
          )} relative rounded-2xl border border-slate-800 bg-surface/70 p-6`}
        >
            ...
            <div className="grid gap-3">
              {[
                ...
                <Link
                  key={item.href}
                  href={item.href}
                  className="group flex items-center justify-between rounded-xl border border-slate-800 bg-slate-900/40 px-4 py-3 text-sm transition hover:-translate-y-0.5 hover:border-slate-600 motion-reduce:transform-none"
                >
                  <span>
                    {item.label}{" "}
                    <span className="text-xs text-slate-400" lang="en">
      ...
      <section className={`mt-16 grid gap-6 md:grid-cols-3 ${heroMotion("delay-500")}`}>
        {copy.entryCards.map((card) => (
          ...
          >
            ...
            <p className="mt-1 text-xs text-slate-400" lang="en">
              {card.en}
            </p>
          </Link>
        ))}
      ...
      <RevealSection className="mt-20">
        ...
        <div className="mt-6 grid gap-4 md:grid-cols-3">
          {blogItems.map((item) => (
            <article
              key={item.title}
              className="rounded-xl border border-slate-800 bg-surface/60 p-5"
            >
