...
| 当前状态 | P3A 已实现 URL + 结构化 Brief、capture harness、model adapter、output schema gate 和 safe fallback |
| 技术前置 | 真实模型接入前必须沿用 `docs/website/AI_PAGE_ANALYSIS_V1_TECH_SPEC.md` 的 SSRF、input schema、output schema gate 和错误码护栏 |
...
| 模型适配器 | 通过 model adapter 边界接入分析能力 | 未配置真实模型时使用 safe fallback，不伪装成真实模型 |
| 输出 schema gate | 对模型或 fallback 输出做结构校验 | 缺少 score、evidence、impact、recommendation 或 backlog 时稳定失败 |
...
| 浏览器渲染抓取 | 先使用 HTML capture harness，避免引入无头浏览器成本和安全面 | V2/V3 |
...
| 4 | 等待结果 | 展示阶段进度：抓取、解析、诊断、生成 | 抓取或模型超时可重试 |
...
| 模型返回不可解析 | `invalid_model_output` | 502 | 展示失败，保留 URL 和 Brief |
...
| safe fallback | 缺少真实模型环境变量时返回 deterministic safe mock output，响应保留 `safe_mock_api: true` |
...
| 项目 | P3A implemented |
|---|---|
| 输入 | URL mode 已使用 Audience、Goal、Problem 三字段 structured brief |
| 分析边界 | `/api/analyze` 通过 model adapter 选择分析能力 |
| 输出 | `output schema gate` 统一校验 scores、issues、recommendations、backlog 和 confidence |
| fallback | 未配置真实模型时使用 safe fallback，仍返回稳定结构和 `safe_mock_api: true` |
| 非目标 | no login、no history、no PDF、no screenshot upload、no browser crawler、no payment |
